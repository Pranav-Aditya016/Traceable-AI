...
const Step: React.FC<{ icon: React.ReactElement; title: string; children: React.ReactNode }> = ({ icon, title, children }) => (
  <div className="flex items-start space-x-4">
    <div className="flex-shrink-0 w-12 h-12 flex items-center justify-center bg-indigo-100 dark:bg-indigo-900/50 rounded-full text-indigo-600 dark:text-indigo-300">
      {icon}
    </div>
    <div>
...
export const PipelineExplanation: React.FC<PipelineExplanationProps> = ({ reportData }) => {
  ...
  return (
    <div className="p-4 bg-white dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700">
      <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-4">How The AI Found The Result</h3>
      <div className="space-y-6">
        <Step
          icon={<IconDocument />}
          title="1. Document OCR"
        >
          The uploaded document was processed using Optical Character Recognition (OCR) to extract the raw text.
        </Step>
        <Step
          icon={<IconExtract />}
          title="2. Key Field Extraction"
        >
          The AI identified key patient information: Age ({fields.age}), Sex ({fields.sex}), and Symptoms ({fields.symptoms.join(', ')}).
        </Step>
        <Step
          icon={<IconPredict />}
          title="3. Condition Prediction"
        >
          Based on the extracted data, the model '{primaryCondition.model}' predicted a '{primaryCondition.label}' in the '{predictions.organ}' with {(primaryCondition.confidence * 100).toFixed(0)}% confidence.
        </Step>
        <Step
          icon={<IconLocalize />}
          title="4. Finding Localization"
        >
        ...
        <Step
          icon={<IconGenerate />}
          title="5. Synthetic Image Generation"
        >
